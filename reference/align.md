...
pick a different metric. `method = "redundancyOverlap"` collapses to
`jaccard` when every pathway has `nSpecies == 1` (Hill-1 saturation). On
hand-rolled single-species inputs the implementation returns `NA` rather
than the algebraically-equivalent Jaccard value; compute Jaccard
...
#> Pathways: 0 shared, 9 query-only, 4 reference-only.
```
