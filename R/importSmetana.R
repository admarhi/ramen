...
#'
#' @param data Either a file path to a single SMETANA TSV, a
#'   directory path containing multiple SMETANA TSVs, or a
#'   pre-loaded data.frame with the required columns
#'   (`receiver`, `donor`, `compound`, and `smetana` if
#'   `use_scores = TRUE`).
#' @param name Consortium (or set) name as a length-1
#'   character scalar. If `NULL` (default), names are derived
#'   from the input filename(s) by stripping the
#'   `.tsv_detailed.tsv` suffix. Required when `data` is a
#'   data.frame. If `data` is a directory, `name` is passed
#'   through to [ConsortiumMetabolismSet()].
#' @param use_scores If `FALSE`, flux is binary (+1 for
#'   production, -1 for consumption). If `TRUE` (default),
#'   flux magnitude is the SMETANA score, aggregated by
#'   `max()` when the same (species, compound) pair appears
#'   across multiple interaction partners.
#' @param normalize_ids If `TRUE` (default), normalize
#'   compound IDs via [.normalizeBiggIds()] (strips `M_`
#'   prefix and `_e` compartment suffix).
#' @param verbose If `TRUE` (default), emit progress messages
#'   via cli progress messages.
#'
#' @return A `ConsortiumMetabolism` object if `data` is a
#'   single file or data.frame; a `ConsortiumMetabolismSet`
#'   if `data` is a directory.
#'
#' @seealso [ConsortiumMetabolism()],
#'   [ConsortiumMetabolismSet()], [importMisosoup()]
#'
#' @export
#'
#' @examples
