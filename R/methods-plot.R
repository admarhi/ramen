...
.plotNetwork <- function(cma) {
    ...
    plotDirectedFlow(
        ...
        main = paste(
            cma@QueryName, "vs", cma@ReferenceName
        )
    )
}
...
.plotScores <- function(cma) {
    ...
    vals <- vapply(s, function(v) {
        if (is.numeric(v) && length(v) == 1L) v
        else NA_real_
    }, numeric(1L))
