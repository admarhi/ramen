...
The alignment classifies every metabolite-metabolite pathway as shared, unique
...
The query network's pathways are shuffled while preserving each metabolite's
...
| `sharedPathways()` | pairwise | data.frame of shared pathways |
...
| `consensusPathways()` | multiple | data.frame (same as prevalence) |
| `metabolites()` | both | Character vector of metabolites |
| `pathways()` | both | data.frame of pathways |
