...
\mathbf{X}^{(\alpha,7)}
```
holds cell-wise exactly (the effective assays are stored at full
precision); same for production. See the *Hill-1 saturation* caveat for
behaviour on small consortia.
...
nEffectiveSpeciesProduction store the Hill-1 perplexity at full
precision. For two-species consortia the perplexity rarely exceeds
$`\approx 1.05`$ (one species typically dominates the flux at any given
pathway), so values cluster just above $`1.0`$ and carry little signal
to distinguish near-degenerate mixtures from true single-species
pathways. Because EffectiveConsumption and EffectiveProduction are the
elementwise product $`F \cdot D_1`$, the same saturation propagates to
them: on small consortia they degenerate toward the underlying
Consumption / Production assays. Both effective views are informative
only on consortia where several species contribute comparable flux to
the same pathway.
...
cma_p <- withr::with_seed(1L, align(
    cm_list[[1]],
    cm_list[[2]],
    method = "FOS",
    computePvalue = TRUE,
...
hits_p <- withr::with_seed(1L, align(
    query,
    db,
    computePvalue = TRUE,
    nPermutations = 99L
