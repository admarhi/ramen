...
usual `ggplot2` operators (e.g. `+ ggplot2::scale_colour_*()`,
`+ ggplot2::labs()`, `+ ggplot2::ggsave(...)`). Network plots are
rendered with the `ggraph` package and use
[`theme_void()`](https://ggplot2.tidyverse.org/reference/ggtheme.html)
as their canonical base; adding `+ theme_minimal()` will *replace* that
and resurrect the underlying layout-coordinate axes (which are not
meaningful), so prefer `+ theme(...)` to tweak specific elements without
replacing the base. Heatmap, dendrogram and scores plots build on
[`theme_ramen()`](https://admarhi.github.io/ramen/reference/theme_ramen.md)
and accept `+ theme_minimal()` cleanly.
...
Nodes are coloured by role from the Okabe-Ito colour-blind-safe palette
in `ramenPalette$nodeRole`: **blue** for sources (only outgoing edges),
**yellow** for sinks (only incoming edges), and **vermilion** for
intermediate nodes (both incoming and outgoing). The `type` argument
selects which assay matrix determines edge weights.
...
The `EffectiveConsumption` and `EffectiveProduction` assays store the
flux scaled by Hill-1 perplexity, $`F \cdot 2^{H(p)}`$: large when total
flux is large *and* species contribute evenly to the pathway. The unit
is the same as `Consumption` / `Production` (a flux). The
`nEffectiveSpeciesConsumption` and `nEffectiveSpeciesProduction` assays
store the perplexity alone (unitless, in $`[1, S]`$) and saturate at 1
on small consortia.
...
![Effective consumption network (flux
units).](visualisation_files/figure-html/plot-cm-effcons-1.png)

Effective consumption network (flux units).
...
![Effective production network (flux
units).](visualisation_files/figure-html/plot-cm-effprod-1.png)

Effective production network (flux units).

``` r

plot(cm_list[[1]], type = "nEffectiveSpeciesConsumption")
```

![Effective number of consuming species (Hill-1 perplexity,
unitless).](visualisation_files/figure-html/plot-cm-neffcons-1.png)

Effective number of consuming species (Hill-1 perplexity, unitless).

``` r

plot(cm_list[[1]], type = "nEffectiveSpeciesProduction")
```

![Effective number of producing species (Hill-1 perplexity,
unitless).](visualisation_files/figure-html/plot-cm-neffprod-1.png)

Effective number of producing species (Hill-1 perplexity, unitless).
...
| CM | `"EffectiveConsumption"` | Consumption flux scaled by Hill-1 perplexity ($`F \cdot 2^{H(p)}`$, flux units) |
| CM | `"EffectiveProduction"` | As above on the production side |
| CM | `"nEffectiveSpeciesConsumption"` | Hill-1 effective number of consuming species (unitless, in $`[1, S]`$) |
| CM | `"nEffectiveSpeciesProduction"` | As above on the production side |
...
#>  [5] viridis_0.6.5                   Biostrings_2.80.1              
...
#> [25] graphlayouts_1.2.5              S4Arrays_1.12.0                
#> [27] DelayedArray_0.38.2             RColorBrewer_1.1-3             
#> [29] TreeSummarizedExperiment_2.20.0 abind_1.4-8                    
#> [31] BiocParallel_1.46.0             withr_3.0.3                    
#> [33] purrr_1.2.2                     BiocGenerics_0.58.1            
...
#> [53] parallel_4.6.1                  BiocManager_1.30.27            
...
#> [63] S4Vectors_0.50.1                ggrepel_0.9.8                  
