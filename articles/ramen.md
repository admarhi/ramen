...
- **Pathway** – in the `ramen` sense, a directed
  ...
  biochemical pathway.
- **Assay** – one of the eight *m x m* metabolite-by-metabolite matrices
  stored in a `ConsortiumMetabolism` object (Binary, nSpecies,
  Consumption, Production, EffectiveConsumption, EffectiveProduction,
  nEffectiveSpeciesConsumption, nEffectiveSpeciesProduction). The two
  `Effective*` slots store flux quantities $`F \cdot 2^{H(p)}`$
  (consumption / production flux scaled by Hill-1 perplexity); the two
  `nEffectiveSpecies*` slots store the unitless perplexity $`2^{H(p)}`$
  alone.
...
data$species <- gsub("[._[:space:]-]+", "_", tolower(data$species))
```
...
The eight assay matrices encode different views of the network:
...
| EffectiveConsumption | Consumption flux scaled by Hill-1 perplexity, $`F \cdot 2^{H(p)}`$ (same units as Consumption) |
| EffectiveProduction | As above on the production side |
...
`EffectiveConsumption` and `nEffectiveSpeciesConsumption` are related by
the exact algebraic identity
`EffectiveConsumption == Consumption * nEffectiveSpeciesConsumption`;
see the *Mathematical formulation* section of the alignment vignette for
...
#> Pathways: 0 pan-cons, 8 niche, 1 core, 8 aux (quantileCutoff = 0.1; buckets may
#> overlap, see ?pathways).
#> Species: 2 generalists, 9 specialists (quantileCutoff = 0.15).
```
...
#> [53] ggforce_0.5.0                   parallel_4.6.1                 
...
#> [67] dendextend_1.19.1               tidyr_1.3.2                    
...
#> [77] rappdirs_0.3.4                  htmltools_0.5.9                
#> [79] Seqinfo_1.2.0                   R6_2.6.1                       
#> [81] textshaping_1.0.5               tidygraph_1.3.1                
#> [83] evaluate_1.0.5                  lattice_0.22-9                 
#> [85] Biobase_2.72.0                  memoise_2.0.1                  
#> [87] bslib_0.11.0                    Rcpp_1.1.2                     
#> [89] gridExtra_2.3.1                 SparseArray_1.12.2             
#> [91] nlme_3.1-169                    xfun_0.60                      
